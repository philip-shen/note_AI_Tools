...
if torch.cuda.is_available():
    logger.info("CUDA installed succesfully\n") 
else:
        logger.info("CUDA not properly installed. Stopping process...")
        quit()
...
'''
def tts_vocal(json_data, text_to, img_fname):
    # Prompt model selection
    # for example, tts_models/multilingual/multi-dataset/xtts_v2
    model = json_data["tts_model"] #"tts_models/multilingual/multi-dataset/xtts_v2"

    # Example voice cloning with selected model
    tts = TTS((model)).to(device)

    out_wav_file_path= "output_{0}_{1:%Y%m%d_%H%M%S}.wav".format(img_fname, datetime.now()) 
    tts.tts_to_file(text_to, speaker_wav= json_data["speaker_wav"],
                    language= "zh", file_path= out_wav_file_path)
    logger.inf(f"out_wav_file_path: {out_wav_file_path}")
    
    time_process = time.time()
    # Extract data and sampling rate from file
    data, fs = sf.read(out_wav_file_path, dtype='float32')  
    sd.play(data, fs)
...
if __name__ == '__main__':
    ...
    for image_file in image_files:
        if image_file not in df['image_file'].values:
            time_process = time.time()
            text_speech= process_image(image_file, model, mediatype=mediatype)
            est_timer(time_process)

            tts_vocal(json_data, text_speech, str(os.path.basename(image_file)) )
